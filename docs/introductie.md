[← Terug naar hoofdpagina](../README.md)

# Introductie
Kader je project in max. 500 woorden.
* Wat is het probleem?
* Hoe groot is dit probleem?
* Wat is de relevantie van dit probleem?
* Wat zijn de deelaspecten van dit probleem (wat is erover geweten)?
* Wat toont onderzoek aan? (refereer naar papers, rapporten, ...). Gebruik websites als [ACM](https://dl.acm.org/) (ACM conferenties en journals), [Google Scholar](https://scholar.google.nl) (of [Scholar Labs](https://scholar.google.nl/scholar_labs/search?hl=nl), of AI-supported academic search tools zoals [Consensus](https://consensus.app/).
* Dit deel leidt toe naar je methododologie waarin je aangeeft hoe je dat probleem op een methodische manier oploste.

Maak gebruik van bronverwijzingen ([zie APA stijl](https://www.scribbr.nl/category/apa-stijl/)). Zoals bijvoorbeeld (Thomas & Ritter, 2022) [^1] .   


[← Terug naar hoofdpagina](../README.md) · [Methodologie →](methodologie.md)
[^1]: Thomas, T., & Ritter, A. (2022). Wandering & sundowning in dementia. _Practical Neurology, 21_(3), 36–44.

